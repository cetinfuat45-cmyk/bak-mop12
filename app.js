        ...
        .onSnapshot((snapshot) => {
            ...
            faultDocs.forEach(fault => {
                ...
                if (cStatus === "Müdahale Ediliyor") {
                    statusLabelHtml = `<div class="fault-status" style="color:#000; background: ${cardBg !== 'var(--surface-color)' ? cardBg : '#3b82f6'}; padding:4px 8px; border-radius:4px; display:inline-block; font-weight:bold;">👷 MÜDAHALE EDİLİYOR (${activeIntervener})</div>`;
...
function v4BuildExportRow(data) {
    ...
    if (Array.isArray(data.interventions) && data.interventions.length) {
        maintenanceLog = data.interventions
            ...
            .map(x => `${(x.operator || 'Bilinmeyen').toLocaleUpperCase('tr-TR')} - ${(x.actionTaken || 'MÜDAHALE ETTİ').toLocaleUpperCase('tr-TR')} (${Number(x.durationMin) || 0} dk)`)
            .join('\n');
    }
