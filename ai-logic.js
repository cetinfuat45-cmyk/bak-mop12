...
window.handleAIBarkodRead = async function(decodedText) {
    ...
    if(aiChatState === 'WAIT_BARCODE_START') {
        const faultsRef = db.collection('arizalar');
        ...
        if (!(await ensureSingleActiveFault(loggedInOperator.name, faultDoc.id, 'main'))) { aiChatState = 'IDLE'; return; }
        ...
        const faultsRef = db.collection('arizalar');
        const snapshot = await faultsRef.where('status', '==', 'Müdahale Ediliyor')
...
window.sendAIChatMessage = async function() {
    ...
    if(aiChatState === 'WAIT_RESOLUTION') {
        ...
        const openRef = await db.collection('arizalar').where('status','in',['Açık','Müdahale Ediliyor']).get();
        const closedRef = await db.collection('arizalar').where('status','==','Kapalı').where('completedAt','>=',todayStart).get();
...
window.finalizeReport = async function(finalText) {
    ...
    try {
        const faultRef = db.collection('arizalar').doc(aiContextData.faultId);
        const aiFaultSnap = await faultRef.get();
        const aiFault = aiFaultSnap.exists ? aiFaultSnap.data() : {};
        const helperCompletionLogs = (aiFault.helpers || []).map(helperName => ({
            operator: helperName, helpers: [], role: 'Yardımcı', durationMin: calculateOperatorSegmentMinutes(aiFault, helperName, 'YARDIMA KATILDI'),
            actionTaken: 'Yardımı tamamladı', logAction: 'YARDIMI TAMAMLADI', status: 'Yardımcı Tamamlandı', timestamp: new Date().toISOString()
        }));
        ...
        try {
            await exportClosedFaultImmediately(aiContextData.faultId);
            addAIMessage("✅ Arıza E-Tabloya kaydedildi ve açık arıza sisteminden silindi. Oluşturduğum rapor:\n" + proReport);
        } catch (transferError) {
            console.error('AI anlık E-Tablo aktarım hatası:', transferError);
            addAIMessage("⚠️ Arıza kapatıldı ancak E-Tablo aktarımı başarısız olduğu için sistemden silinmedi. Hata: " + transferError.message + "\nRapor:\n" + proReport);
        }
